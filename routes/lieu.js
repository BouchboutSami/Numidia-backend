...
router.get('/moyen/:nomlieu', async (req,res)=>{
      let  nomlieu =  req.params.nomlieu
      ...
      if( data == null ){ res.send('no data found')} 
          ...
          if( avis.length == 0 ){ res.json({'message':'lieu non noté' })}
          else{
...
router.get('/comments/:nomlieu', async (req,res)=>{
      let nomlieu = req.params.nomlieu
      ...
      if( data == null ){ res.send('no data found')} 
          ...
          if(comments.length == 0 ){res.json({message: 'no commentaire'})}
          else{
 ...
 router.post('/add/:idadmin' ,upload.array('pictures', 5) ,async(req, res)=>{
        ...
        try {
          const  {nom ,theme ,categorie,description ,tarif , documentation,tempsouverture, tempsfermeture,lat,long,wilaya} = req.body
 ...
 router.get('/pictures/:idlieu' , async (req,res)=>{
      ...
      try {
       ...
       if (pictures == null) {return res.status(204).json({message: 'no pictures for this place'})}
      else{
        res.send(pictures)
      }
      } catch (error) {
